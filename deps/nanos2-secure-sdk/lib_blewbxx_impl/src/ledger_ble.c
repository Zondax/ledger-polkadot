...
typedef enum {
	...
	BLE_CONFIG_ADV_STEP_SET_ADV_PARAMETERS,
...
static void configure_advertising_mngr(uint16_t opcode)
{
	...
	else if (ledger_ble_data.adv_step == BLE_CONFIG_ADV_STEP_START) {
		ledger_ble_data.advertising_enabled = 1;
	}

	ledger_ble_data.adv_step++;
	if (  (ledger_ble_data.adv_step == BLE_CONFIG_ADV_STEP_START)
